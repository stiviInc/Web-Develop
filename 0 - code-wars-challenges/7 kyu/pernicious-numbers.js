...
function pernicious(n) {
  var perniciousNumbers = [];
  n = Math.floor(n);
  if (n > 2) {
    var cont = 2;
    while (cont <= n) {
      var sum = 0;
      var numberInBinary = cont.toString(2);
      var aux = 0;
      while (aux < numberInBinary.length) {
        sum += parseInt(numberInBinary[aux], 2);
        aux++;
      }
      //console.log("Suma: " + sum);
      //console.log(cont.toString(2));

      if (itsPrime(sum) == true) perniciousNumbers.push(cont);
...
function itsPrime(number) {
  ...
  for (var divisor = 1; divisor <= number; divisor++) {
    if (number % divisor === 0) veces++;
  }

  return veces == 2 ? true : false;
}

console.log(pernicious(55));
